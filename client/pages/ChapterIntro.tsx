...
import { ChevronLeft, ChevronRight, BookOpen, Zap, AlertCircle, ChevronDown } from "lucide-react";
...
export default function ChapterIntro() {
  ...
  // Detect if title text is truncated
  useEffect(() => {
    if (titleRef.current) {
  ...
  const handleTitleClick = (itemId: string) => {
    setExpandedTitleId(expandedTitleId === itemId ? null : itemId);
  };
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      <h2
                        ref={titleRef}
                        className={`text-base sm:text-2xl font-bold text-foreground transition-all flex-1 ${
                          expandedTitleId === currentItem.data.id ? '' : 'truncate'
                        }`}
                        onClick={() => handleTitleClick(currentItem.data.id)}
                        style={{ cursor: isTitleTruncated ? 'pointer' : 'default' }}
                      >
                      ...
                      {isTitleTruncated && (
                        <ChevronDown
                          className={`w-5 h-5 flex-shrink-0 text-muted-foreground transition-transform ${
                            expandedTitleId === currentItem.data.id ? 'rotate-180' : ''
                          }`}
                        />
                      )}
                    </div>
            ...
            <div key={currentItem.data.id} className="animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              <div className="card-interactive mb-3 sm:mb-4 p-3 sm:p-6">
                <h2 className="text-base sm:text-xl font-bold text-foreground mb-2 sm:mb-3">
                  {parseContentWithTerms((currentItem.data as any).description)}
                </h2>
                        ...
                        <div className="flex items-start gap-2 sm:gap-3">
                          ...
                          <div className="flex-1 text-left">
                            <p className="font-semibold text-sm">
                              {parseContentWithTerms(option.text)}
                            </p>
                            {selectedOption === option.id && (
                      ...
                      {showFeedback && selectedOption === option.id && (
                        ...
                        >
                          ...
                          <p className="text-xs sm:text-sm text-foreground mt-1">
                            {parseContentWithTerms(option.feedbackText)}
                          </p>
                        </div>
                      )}
                    </div>
